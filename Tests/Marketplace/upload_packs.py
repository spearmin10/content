...
def get_packs_names(target_packs):
    ...
    if target_packs.lower() == "all":
        if os.path.exists(PACKS_FULL_PATH):
            ...
            logging.error((f"Folder {PACKS_FOLDER} was not found "
...
def download_and_extract_index(storage_bucket, extract_destination_path):
    ...
    if os.path.exists(download_index_path):
        ...
        if not os.path.exists(index_folder_path):
            logging.error(f"Failed creating {GCPConfig.INDEX_NAME} folder with extracted data.")
            sys.exit(1)
        ...
        logging.error(f"Failed to download {GCPConfig.INDEX_NAME}.zip file from cloud storage.")
        sys.exit(1)
...
def clean_non_existing_packs(index_folder_path, private_packs, storage_bucket):
    ...
    if invalid_packs_names:
        try:
            ...
            logging.exception(f"Failed to cleanup non existing packs.", e)
...
def upload_index_to_storage(index_folder_path, extract_destination_path, index_blob, build_number, private_packs,
    ...
    try:
        ...
        if current_index_generation == index_generation:
            ...
            logging.info(f"Finished uploading {GCPConfig.INDEX_NAME}.zip to storage.")
        else:
        ...
        logging.exception(f"Failed in uploading {GCPConfig.INDEX_NAME}", e)
        sys.exit(1)
    finally:
...
def upload_core_packs_config(storage_bucket, build_number, index_folder_path):
    ...
    for pack in os.scandir(index_folder_path):
        if pack.is_dir() and pack.name in GCPConfig.CORE_PACKS_LIST:
            ...
            if not os.path.exists(pack_metadata_path):
                logging.error(f"{pack.name} pack {Pack.METADATA} is missing in {GCPConfig.INDEX_NAME}")
                sys.exit(1)
            ...
            if not storage_bucket.blob(core_pack_relative_path).exists():
                logging.error(f"{pack.name} pack does not exist under {core_pack_relative_path} path")
                sys.exit(1)
    ...
    if len(found_core_packs) != len(GCPConfig.CORE_PACKS_LIST):
        ...
        logging.error(f"Missing core packs are: {missing_core_packs}")
        sys.exit(1)
    ...
    logging.info(f"Finished uploading {GCPConfig.CORE_PACK_FILE_NAME} to storage.")
...
def upload_id_set(storage_bucket, id_set_local_path=None):
    ...
    logging.info("Finished uploading id_set.json to storage.")
...
def get_private_packs(private_index_path):
    ...
    for metadata_file_path in metadata_files:
        try:
            ...
            logging.exception(f'Invalid JSON in the metadata file [{metadata_file_path}].', e)
...
def update_index_with_priced_packs(private_storage_bucket, extract_destination_path, index_folder_path):
    ...
    try:
        ...
        logging.exception(f'Could not add private packs to the index.', e)
    finally:
...
def check_if_index_is_updated(index_folder_path, content_repo, current_commit_hash, remote_previous_commit_hash,
    ...
    try:
        ...
        logging.exception(f"Failed in checking status of index", e)
        sys.exit(1)
...
----------------------------------------------------------------------------------------------------------""")
    ...
    if successful_packs:
        ...
        logging.info(f"Number of successful uploaded packs: {len(successful_packs)}")
        logging.info(f"Uploaded packs:\n{successful_packs_table}")
    if skipped_packs:
    ...
    if failed_packs:
        ...
        logging.error(f"Number of failed packs: {len(failed_packs)}")
        logging.error(f"Failed packs:\n{failed_packs_table}")
        sys.exit(1)
...
def add_pr_comment(comment):
    ...
    try:
        ...
        if res and res.get('total_count', 0) == 1:
            ...
            logging.warning('Add pull request comment failed: There is more then one open pull request for branch {}.'
                          .format(branch_name))
    except Exception as e:
        logging.warning('Add pull request comment failed: {}'.format(e))


def handle_github_response(response):
    res_dict = response.json()
    if not res_dict.get('ok'):
        logging.warning('Add pull request comment failed: {}'.
                      format(res_dict.get('message')))
    return res_dict


def main():
    install_logging('Prepare Content Packs For Testing')
